 ...
 * }
 * 
 * 	- if문과 다르게 범위형태(조건식)으로 분기 할 수 없고
 * 	  일차적으로 값을 기준으로 분기한다.
 *  - switch의 기준변수에는 정수 , 문자 , 문자열 자료형을 사용할 수 있다.
 *  - 분기되는 case뿐만 아니라 그 case의 아래 case까지 모두 수행한다.
 *    이것을 원치 않을 경우 의도적으로 보조제어문 break 명령어를 함께 작성한다.
 *  - default는 case에 해당하는 값이 경우 없을 경우 분기하는 구문이다.  
 *  
 * 
 * 
 * 
